...
```bash
node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" generate-claude-md
```
...
```bash
node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md CLAUDE.md
```
...
```bash
PHASE1_SECTION=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" roadmap get-phase 1 2>/dev/null)
