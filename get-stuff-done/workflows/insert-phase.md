...
```bash
INIT=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
