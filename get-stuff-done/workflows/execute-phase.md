...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/get-stuff-done/workflows/execute-plan.md` **inline**
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
Commit the file:
```bash
node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
