...
```bash
DEBT=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
