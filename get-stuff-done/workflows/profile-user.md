...
Key references:
- @$HOME/.claude/get-stuff-done/references/ui-brand.md (display patterns)
- @$HOME/.claude/get-stuff-done/agents/gsd-user-profiler.md (profiler agent definition)
- @$HOME/.claude/get-stuff-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/get-stuff-done/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/get-stuff-done/USER-PROFILE.md" "$HOME/.claude/get-stuff-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/get-stuff-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/get-stuff-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/get-stuff-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `$HOME/.claude/get-stuff-done/references/user-profiling.md`
...
Reference: @$HOME/.claude/get-stuff-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/get-stuff-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/get-stuff-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $HOME/.claude/get-stuff-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/get-stuff-done/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/get-stuff-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node $HOME/.claude/get-stuff-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/get-stuff-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/get-stuff-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH="$HOME/.claude/get-stuff-done/USER-PROFILE.backup.md"
```
...
Your profile:    $HOME/.claude/get-stuff-done/USER-PROFILE.md
```
