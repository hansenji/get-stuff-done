...
ワークフローファイル（`get-stuff-done/workflows/*.md`）は重い処理を行いません。以下の役割に徹します：
...
### ワークフロー（`get-stuff-done/workflows/*.md`）
...
### リファレンス（`get-stuff-done/references/*.md`）
...
### テンプレート（`get-stuff-done/templates/`）
