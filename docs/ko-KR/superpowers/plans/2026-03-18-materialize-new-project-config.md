...
| 파일 | 액션 | 목적 |
|------|--------|---------|
| `get-stuff-done/bin/lib/config.cjs` | 수정 | `buildNewProjectConfig()` + `cmdConfigNewProject()` 추가 |
| `get-stuff-done/bin/gsd-tools.cjs` | 수정 | `config-new-project` case 등록 + usage 문자열 업데이트 |
| `get-stuff-done/workflows/new-project.md` | 수정 | Steps 2a + 5: 인라인 JSON 작성을 CLI 호출로 교체 |
...
- 수정: `get-stuff-done/bin/lib/config.cjs`
...
`get-stuff-done/bin/lib/config.cjs`에서 `validateKnownConfigKeyPath` 함수 뒤(약 35번째 줄)와 `ensureConfigFile` 앞에 다음을 추가하세요:
...
- 수정: `get-stuff-done/bin/gsd-tools.cjs`
...
- 수정: `get-stuff-done/workflows/new-project.md`
